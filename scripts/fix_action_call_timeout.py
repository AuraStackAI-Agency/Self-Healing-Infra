#!/usr/bin/env python3
"""Fix the timeout on Appeler Action Executor - use fire-and-forget pattern"""
import requests
import json
...
# Fix Appeler Action Executor - increase timeout significantly
for node in w['nodes']:
    if node['name'] == 'Appeler Action Executor':
        node['parameters']['options'] = {
            'timeout': 300000,  # 5 minutes - enough for full execution
            'retry': {'maxRetries': 1, 'waitBetweenRetries': 5000}
        }
        print("[OK] Increased timeout to 5 minutes")

    if node['name'] == 'Escalader vers N2':
        node['parameters']['options'] = {
            'timeout': 300000,  # 5 minutes
            'retry': {'maxRetries': 1, 'waitBetweenRetries': 5000}
        }
        print("[OK] Increased N2 escalation timeout")
