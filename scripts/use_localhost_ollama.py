#!/usr/bin/env python3
"""Change Ollama URL to localhost if N8N is on same server"""
import requests
import json
...
# Change Ollama URL to localhost
for node in w['nodes']:
    if node['name'] == 'Ollama - Qwen N1':
        # Try localhost since N8N might be on same server
        node['parameters']['url'] = "http://localhost:11434/api/generate"
        node['parameters']['options'] = {
            'timeout': 180000,  # 3 minutes
            'retry': {'maxRetries': 1, 'waitBetweenRetries': 10000}
        }
        print("[OK] Changed Ollama URL to localhost:11434")
        print("[OK] Increased timeout to 180s")
