#!/usr/bin/env python3
"""Fix SSH - Docker Logs to use $json instead of expression"""
import requests
import json
...
# Fix SSH - Docker Logs to use $json directly
for node in w['nodes']:
    if node['name'] == 'SSH - Docker Logs':
        # Use $json instead of $('Normaliser Payload') reference
        node['parameters']['command'] = "=docker logs --tail 50 {{ $json.service_name }} 2>&1 || echo 'No Docker container named {{ $json.service_name }}'"
        print("[OK] Fixed SSH - Docker Logs - now uses $json.service_name")
